...
export default function CustomBtnLogin() {
    ...
    return (
      <>
        <div className="grid justify-items-center">
          <img src={session.user.image} className="w-32 rounded-full mb-4" />
          Welcome {session.user.name} <br />
          <div className="">
            <button
              className="bg-red-600 p-2 mt-4  rounded-lg text-white hover:bg-red-400 mr-12 hover:border-2 hover:border-gray-500"
              onClick={() => {
                signOut();
              }}
            >
